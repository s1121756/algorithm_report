...
//In this memory game, I use the Fisher shuffle algorithm to mix the letter pairs. 
//This makes sure that the positions are random and fair every time we play. 
//I used shuffle() with mt19937 to do this, so the game feels different and fun each time.
template <typename T>
...
//It shows opened letters and hides the rest with blank spaces. 
//Every five elements, it starts a new line to make the board look clean and easy to read for the player.
template <typename T>
...
//This function asks the player to choose a position on the board by entering a number. 
//It checks if the input is valid—such as not out of range, not a copy selection, and not already revealed. 
//If the input is invalid, it resets and asks again. This ensures smooth gameplay and prevents mistakes.
template <typename T>
...
//If it's the first selection, it simply returns true and waits for the second card. 
//Once two cards are selected, it compares their values. 
//If they match, it keeps them revealed; 
//if not, it hides them again and resets the selection state.
template <typename T>
 ...
 */
 ...
 //This function handles the main game logic after each user input. 
 //It checks if the selected card matches the previous one and updates the display accordingly. 
 //If all pairs are found, it asks the user if they want to play again. 
 //If not, it exits the game. 
 //Otherwise, it continues by asking for the next input and recursively calling itself to repeat the process.
template <typename T>
